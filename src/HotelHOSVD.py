...
df = pd.read_csv("{0}\data\hotelReviews24k.csv".format(os.getcwd()))

allColumns = ["Hotel_ID", "Users_ID", "Value", "Rooms", "Location", 
    "Cleanliness", "CheckInFrontDesk", "Service", "BusinessService"]

columnsCriterion = ["Value", "Rooms", "Location", "Cleanliness", 
    "CheckInFrontDesk", "Service", "BusinessService"]

columnsForCalssification = ["Hotel_ID", "User_Index",
...
items = ListHelperUnique(df["Hotel_ID"].to_numpy())
users = ListHelperUnique(df["Users_ID"].to_numpy())
...
path = "{0}\data\singular_vectors_hotel_reviews.txt".format(os.getcwd())
np.savetxt(path, singular_vectors[1])
singular_vectors_hotel_reviews = np.loadtxt(path)

path = "{0}\data\matrixForCalssification.txt".format(os.getcwd())
